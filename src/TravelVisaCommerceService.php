...
/** Tenant-neutral order state; VazinPay readback remains the financial authority. */
final class TravelVisaCommerceService
{
    ...
    public function create(string $tenant,string $reference,string $service,?int $customer,string $amount,string $currency): array {
        ...
        $s->execute(['tenant'=>$tenant,'ref'=>$reference,'service'=>$service,'customer'=>$customer,'amount'=>$amount,'currency'=>$currency]); return $this->get($reference);
    }
    public function bindInvoice(string $reference,string $invoiceId,string $key): array { return $this->mutate($reference,'invoice.bound',$key,'system',function(array $o)use($invoiceId):array{if($o['invoice_id']!==null&&$o['invoice_id']!==$invoiceId)throw new RuntimeException('invoice_binding_conflict');return ['invoice_id'=>$invoiceId];}); }
    public function applyReadback(string $reference,array $invoice,string $delivery): array { return $this->mutate($reference,'pay.readback',$delivery,'vazinpay',function(array $o)use($invoice):array{if(($invoice['external_order_id']??'')!=='travelvisa:'.$o['public_ref']||($invoice['amount']??'')!==$o['amount']||($invoice['currency']??'')!==$o['currency'])throw new RuntimeException('pay_readback_mismatch');$status=(string)($invoice['status']??'');if($status==='paid')return ['payment_status'=>'paid','lifecycle_status'=>'paid'];if($status==='refunded')return ['payment_status'=>'refunded','lifecycle_status'=>'cancelled'];throw new RuntimeException('pay_readback_unconfirmed');}); }
